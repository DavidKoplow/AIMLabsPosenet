...
import {drawBoundingBox, drawKeypoints, drawPoint, drawSkeleton, isMobile, toggleLoadingUI, tryResNetButtonName, tryResNetButtonText, updateTryResNetButtonDatGuiCss} from './demo_util';
      ...
      if (score >= minPoseConfidence) {
        if (guiState.output.showPoints) {
          for(var j = 0; j<keypoints.length; j++){
            ...
            if(i.part=="rightWrist" ){
              if (i.score < minPoseConfidence) {
                continue;
              }
              console.log(i)

              const {y, x} = i.position;

              drawPoint(ctx,y,x,y/10);
